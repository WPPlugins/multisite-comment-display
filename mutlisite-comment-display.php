...
<p align="center"><strong>Copyright &copy; ' . $xstartyear . '  - ' . date("Y") . ' by ' . $xname . ' and ' . $xcompanylink1 ;
	...
	// ----------------------------------------------------------------------------
	// filter to get back to the comment list in admin after editing a comment
	//add_filter( 'comment_edit_redirect', 'mcd_return_link' , 6, 2);

	function mcd_comment_display($atts = "") {
	// ----------------------------------------------------------------
	...
	// ----------------------------------------------------------------
	function mcd_comment_edit($atts = "") {
		if (! is_super_admin()) {
		...
		// these scripts to support comment page edit mode
...
//	 echo "SQL = " . $comments_query->request;
...
if ( !empty( $comments ) ) {
    foreach ( $comments as $comment ) {
		...
		if (is_super_admin()) {
			...
			//$xlink = str_replace("wp-admin",  $xsitepath. "wp-admin", $xlink);
			//echo $xreturnlink . '<br>';
			//echo "<br> $xlink <br>";
			echo  '&nbsp;&nbsp;<a href="' . $xlink . '" target="_blank" title="Edit Comment" class="dashicons dashicons-edit"></a>&nbsp;&nbsp;';

//			echo "http://www.cellarweb.com/multisitetest/site2/wp-admin/comment.php?action=editcomment&c=" . $comment->comment_id ; 
		}
		echo "<br>";
